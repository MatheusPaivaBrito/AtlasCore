...
from shared_kernel.http.crud.errors import CrudErrorFactory, DefaultCrudErrorFactory
...
class CrudHandlerBase(Generic[ModelT]):
    ...
    error_factory: ClassVar[CrudErrorFactory] = DefaultCrudErrorFactory()
    ...
    def _load(self, resource_id: UUID, *, include_deleted: bool = False) -> ModelT:
        ...
        if instance is None:
            raise self.error_factory.not_found(entity=self.entity_name, resource_id=resource_id)
    ...
    def _persist(self, instance: ModelT) -> ModelT:
        try:
            ...
            raise self.error_factory.conflict(entity=self.entity_name) from exc
        return instance

    def _column(self, field_name: str) -> Any:
        column = getattr(self.model, field_name, None)
        if column is None:
            raise self.error_factory.unsupported_filter(entity=self.entity_name, field=field_name)
        return column
    ...
    def _coerce_filter_value(self, field_name: str, raw_value: str) -> Any:
        ...
        if python_type is UUID:
            try:
                ...
                raise self.error_factory.invalid_filter_value(
